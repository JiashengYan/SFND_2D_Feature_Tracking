...
//NMS
cv::Mat PerformNMS(cv::Mat corner_img, int sw_size){
    int sw_dist = floor(sw_size/2);
    cv::Mat result_img = cv::Mat::zeros(corner_img.rows, corner_img.cols, CV_8U);
    for (int r = sw_dist; r < corner_img.rows - sw_dist - 1; r++){
        for (int c = sw_dist; c < corner_img.cols - sw_dist - 1; c++){
...
void detKeypointsHarris(std::vector<cv::KeyPoint> &keypoints, cv::Mat &img, bool bVis)
{
    ...
    // dst_norm_NMS = PerformNMS(dst_norm, 3);
    // // Look for prominent corners and instantiate keypoints
    // for (size_t j = 0; j < dst_norm_NMS.rows; j++){
    //     for (size_t i = 0; i < dst_norm_NMS.cols; i++){
    //         if(dst_norm_NMS.at<unsigned int>(j, i) > 0){
    //             cv::KeyPoint newKeyPoint;
